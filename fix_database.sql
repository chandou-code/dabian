-- 修复数据库字符编码并插入测试数据
-- 使用方法：mysql -u root -p123456 < fix_database.sql
...
-- 5. 验证数据插入和排序
SELECT '=== 验证最新物品排序（应该按时间降序）===' as info;
SELECT 
    id,
    item_name as title,
    description,
...
-- 6. 检查字符编码
SELECT '=== 检查字符编码（应该显示正常中文）===' as info;
SELECT 
    id,
    item_name,
    description,
