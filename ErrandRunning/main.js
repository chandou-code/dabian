...
function checkUrlToken() {
  ...
  if (token) {
    ...
    // 从token中获取用户信息
    uni.request({
      url: 'http://localhost:18083/api/auth/user/profile',
      method: 'GET',
      header: {
      ...
      success: (res) => {
        ...
        if (res.data && res.data.code === 200 && res.data.data) {
          ...
          // 2秒后跳转到用户个人资料页面
          console.log('=== 2秒后跳转到用户个人资料页面 ===')
          setTimeout(() => {
            const redirectUrl = window.location.origin + window.location.pathname + '#/pages/user/profile'
            console.log('跳转到用户个人资料页面:', redirectUrl)
